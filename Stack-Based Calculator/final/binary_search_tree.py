...
class BinarySearchTree():
    ...
    def display_tree(self, node=None, level=0, is_root=True):
        """
        Recursively displays the BST structure visually in the output
        Helps to visualize the arrangement of nodes in the tree
        This will display right, root and then left
        """
        #initializes starting point (if this is the initial call from main, set the starting node to the current tree instances root)
        if is_root:
            node = self.root
        
        #if we have hit an empty node, stop this branch of recursion and return back to its caller
        if node is None:
            return
        
        #dive into the right subtree first, since the console prints top to bottom,
        #this makes sure larger alphabetical keys appear at the top of the display.
        if node.right: #if the node has a path to the right
            self.display_tree(node.right, level + 1, False) #recursively call this method again and again to the right until we reach the bottom (hit none)
    
        #creates a staircase by multiplying spaces by the current level of the tree
        indent = "  " * level  
        print(f"{indent}||==> {node.key}:{node.value}") #print the indent created based on level of tree and print the nodes key and value

        #finally, dive into the left subtree, these nodes will appear further down
        #in the console output, representing the lower alphabetical keys
        if node.left: #if there is a path to the left 
            self.display_tree(node.left, level + 1, False) #recursively call the method on the left subtree until we reach the bottom
